...
export default function LiveAuctionPage() {
  ...
  const [realTeamsMap, setRealTeamsMap] = useState<Record<string, any>>({})
  ...
  useEffect(() => {
    ...
    async function initAuctionRoom() {
      ...
      // Carichiamo anche la mappa delle squadre reali (tabella teams) basata sull'alias
  ...
  // Funzione di utilità per renderizzare il badge identico all'anagrafica
  const renderTeamBadge = (teamAlias: string, sizeClass = "w-10 h-10", textClass = "text-sm") => {
    const teamInfo = realTeamsMap[teamAlias]
    const colors = teamInfo?.colors
    const color = teamInfo?.color

    const backgroundStyle = colors && colors.length > 1 
      ? `linear-gradient(135deg, ${colors.join(', ')})` 
      : (color || colors?.[0] || '#1e293b')

    return (
      <div 
        className={`${sizeClass} rounded-xl flex items-center justify-center shrink-0 shadow-md overflow-hidden`}
        style={{ background: backgroundStyle }}
      >
        <span className={`font-black ${textClass} text-white drop-shadow-md tracking-wider uppercase`}>
          {teamAlias ? teamAlias.slice(0, 3).toUpperCase() : ''}
        </span>
      </div>
    )
  }
            ...
            <div className="bg-slate-800/60 border border-slate-700/80 rounded-2xl p-6 md:p-8 space-y-6">
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  {/* Badge squadra reale sotto il nome */}
                  <div className="flex items-center gap-2.5 mt-3">
                    {renderTeamBadge(currentNomination.players?.team, "w-8 h-8", "text-xs")}
                    <span className="text-sm font-semibold text-slate-300">
                      {realTeamsMap[currentNomination.players?.team]?.name || currentNomination.players?.team}
                    </span>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                ...
                {availableTeamsList.map((teamName) => (
                  <option key={teamName} value={teamName}>{realTeamsMap[teamName]?.name || teamName}</option>
                ))}
              </select>
            </div>
                ...
                availablePlayers.map(p => (
                  <div key={p.id} className="flex justify-between items-center p-3 bg-slate-800/80 border border-slate-700/60 rounded-xl">
                    <div className="flex items-center gap-3">
                      {renderTeamBadge(p.team, "w-10 h-10", "text-sm")}
                      <div>
                        <span className="font-bold text-sm text-white block">{p.name}</span>
                        <span className="text-xs text-slate-400">{realTeamsMap[p.team]?.name || p.team}</span>
                      </div>
                    </div>
                    <button 
                      disabled={isSubmitting}
