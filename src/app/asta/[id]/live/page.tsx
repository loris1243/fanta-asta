    ...
    useEffect(() => {
        ...
        async function initAuctionRoom() {
            ...
            for (const c of activeChannels) {
                if (c.topic?.includes(`auction-room-${id}`)) {
                    await supabase.removeChannel(c)
                }
            }

            const channel = supabase.channel(`auction-room-${id}-${Date.now()}`)
                .on(
                    'postgres_changes',
                ...
                .on(
                    ...
                    () => {
                        if (isMounted) fetchCurrentNomination()
                    }
                )
                .subscribe()
                        ...
                        <div className="bg-slate-800/60 border border-slate-700/80 rounded-2xl p-6 md:p-8 space-y-6">
                            ...
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    {/* Blocco squadra reale, logo, alias e bande verticali dei colori */}
                                    <div className="flex items-center gap-3 mt-2">
                                        {(() => {
                                            ...
                                            // Gestione dei colori (può essere un array JS o una stringa)
                                            ...
                                            return (
                                                <>
                                                    {/* Spazio per le bande verticali dei colori */}
