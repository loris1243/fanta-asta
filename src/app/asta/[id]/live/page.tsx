...
export default function LiveAuctionPage() {
    ...
    const loadUserTargets = async () => {
        ...
        if (targetsData) {
            setTargetPlayerIds(
                new Set(
                    targetsData.map((t: any) => t.player_id)
                )
            )
        }
    }
    ...
    const fetchCurrentNomination = async () => {
        ...
        // Il prezzo visualizzato all'inizio dell'asta
        // deve essere il prezzo base della chiamata.
        const initialBid =
        ...
        setHighestTeamId(
            nomination.highest_bidder_team_id || null
        )
    ...
    // ============================================================
    // CALCOLO MIGLIOR OFFERTA ATTIVA
    // ============================================================
    ...
    const getBestActiveBid = (
        ...
        for (const bid of bidRows) {
            if (withdrawnIds.has(bid.team_id)) {
                continue
            }
    ...
    const finalizeAuctionItem = async () => {
        ...
        try {
            const { data: freshNomination, error } =
                await supabase
                    .from('auction_nominations')
                    .select(
            ...
            const withdrawnIds = new Set<string>(
                (freshWithdrawals || []).map(
                    (row: any) => row.team_id
                )
            )

            const { teamId: winningTeamId, amount: winningAmount } =
                getBestActiveBid(
                    (freshBids || []) as BidRow[],
                    withdrawnIds
                )

            // Se nessuno ha ancora fatto un'offerta,
            // utilizziamo il prezzo base e la squadra rimasta
            // come vincitrice solo se c'è una sola squadra attiva.
            let finalWinningTeamId = winningTeamId
            let finalWinningAmount = winningAmount

            const activeTeams = teamsData.filter(
                team => !withdrawnIds.has(team.id)
            )
    ...
    const handleNominatePlayer = async (
        ...
        try {
            ...
            // Recuperiamo la nomination appena creata
            const { data: newNomination, error: nominationError } =
                await supabase
                    .from('auction_nominations')
                    .select('id')
                    .eq('auction_id', id)
            ...
            if (nominationError || !newNomination) {
                throw nominationError || new Error(
                    'Impossibile recuperare la nuova asta.'
                )
            }

            // Impostiamo il prezzo base scelto nella chiamata.
            // All'inizio dell'asta current_bid coincide con il prezzo base.
            const { error: priceError } =
                await supabase
                    ...
                    .update({
                        ...
                        highest_bidder_team_id: currentTurnTeamId
                    })
    ...
    const handlePlaceBid = async (
        ...
        const { data: bidRow, error: bidInsertError } =
            await supabase
                .from('auction_bids')
                .insert({
                    auction_id:
        ...
        // Poi aggiorniamo la nomination corrente
        // per avere immediatamente il miglior offerente.
        const { error: nominationError } =
            await supabase
                .from(
                    'auction_nominations'
                )
                        ...
                        <div className="bg-slate-800/80 border border-slate-700 rounded-xl p-4">
                            ...
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    disabled={isSubmitting}
                                    onChange={(e) =>
                                        setBasePriceValue(e.target.value)
                                    }
