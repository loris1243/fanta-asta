...
export default function LiveAuctionPage() {
  ...
  const fetchParticipantsAndTeams = async () => {
    ...
    if (participants) {
      setTeamsData(participants.map((p: any) => p.league_teams).filter(Boolean))
    }
  }
  ...
  const fetchCurrentNomination = async () => {
    ...
    setCurrentBid(nomination?.current_bid || nomination?.base_price || 0)
            ...
            <div className="bg-slate-800/60 border border-slate-700/80 rounded-2xl p-6 md:p-8 space-y-6">
              <div className="flex justify-between">
                ...
                <div className="text-right">
                  ...
                  <span className="text-5xl font-black text-amber-400 block">{currentBid}</span>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <button onClick={() => handlePlaceBid(1)} className="py-3 bg-slate-700 hover:bg-slate-600 rounded-xl font-bold uppercase text-sm transition">+1</button>
                <button onClick={() => handlePlaceBid(5)} className="py-3 bg-slate-700 hover:bg-slate-600 rounded-xl font-bold uppercase text-sm transition">+5</button>
                <button onClick={() => handlePlaceBid(10)} className="py-3 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold uppercase text-sm transition">+10</button>
              </div>
            </div>
          ) : (
